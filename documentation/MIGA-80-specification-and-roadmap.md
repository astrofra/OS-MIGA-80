...
**2026-09-12 hosted workflow checkpoint:** F5 now emits guarded O1 code, bounds
...
when their result is dead, preserves observable `pset` calls, moves values live
across calls out of caller-saved registers, preserves used saved registers, and renders assembly
...
maximum callee stack use, and controlled fault source locations. The shipping
direct encoder covers O0 and the O1 numeric/`pset` vertical subset: its
404-byte Mandelbrot image and 36-byte call-survival image are byte-identical to
their GNU routes and execute under Musashi, while the former also executes from
the bootable ADF. User calls, broader direct O1 instruction coverage,
ELF/symbol-manifest loading, and hardware performance validation remain
pending.
...
3. **ABI freeze — early Phase 3 (register/stack, exact-width scalar, fixed-point, immutable value, source-local, loop-edge, spill-frame, numeric-fault, and first runtime-call tranches implemented):** ABI 0.6 fixes register roles, `A5` runtime-context ownership, scalar, string, and `void` returns, canonical `bool`/`i8`/`u8`/`i16`/`u16`/`i32`/`fix`/`symbol` register and slot representations, canonical string descriptors, mixed register classes, per-width wrapping, Q16.16 multiplication/division and explicit conversion semantics, frame bounds, stack alignment, executable saved-register checks, and runtime-context offset zero for a non-returning controlled fault handler. Fault codes 1 and 2 identify numeric division by zero and conversion out of range. The compatible graphics profile adds the trusted `pset` service at `4(A5)` and its private pixel state at `8(A5)`; the service may clobber `D0-D2/A0-A1`. `-O0` assigns typed source locals to bounded `A6` slots; `-O1` renames locals through cyclic control flow, keeps call-crossing values in preserved locations, and emits bounded `A6` spill/edge frames when needed while restoring `D3-D7/A6`. Ordinary user calls, cartridge-wide pool linking, later context entries, and later fault codes remain open extensions; multiple returns are excluded from version 1.
